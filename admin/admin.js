...
function logout() {
    ...
    // Uncomment the next line if you want to clear saved credentials on logout
...
// Add function to clear saved credentials
...
function updateVisitorStats(visitors) {
    ...
    const totalPageViews = visitors.reduce((sum, v) => {
        if (!v.pageViews) return sum;
        return sum + (typeof v.pageViews === 'object' ? 
            Object.values(v.pageViews).reduce((a, b) => a + (b.count || 0), 0) : 0);
    }, 0);
...
function displayVisitorList(visitors) {
    ...
    visitorList.innerHTML = sortedVisitors.map(visitor => `
        <tr>
            <td>${visitor.visitorId || 'Unknown'}</td>
            <td>${visitor.ipAddress || 'Unknown'}</td>
            <td>${formatLocation(visitor.location)}</td>
            <td>${new Date(visitor.startTime || 0).toLocaleString()}</td>
            <td>${formatTime(visitor.timeSpent || 0)}</td>
...
function showVisitorDetails(visitorId) {
    ...
    document.getElementById('basicInfo').innerHTML = `
        ...
        <p><strong>First Visit:</strong> ${new Date(visitor.startTime || 0).toLocaleString()}</p>
        <p><strong>Last Active:</strong> ${new Date(visitor.lastActive || 0).toLocaleString()}</p>
        <p><strong>Time Spent:</strong> ${formatTime(visitor.timeSpent || 0)}</p>
    `;
        ...
        <table class="detail-table">
            ...
            ${Object.entries(visitor.pageViews || {}).map(([page, data]) => `
                <tr>
                    <td>${page || '/'}</td>
                    <td>${data.count}</td>
                    <td>${new Date(data.firstVisit || 0).toLocaleString()}</td>
                    <td>${new Date(data.lastVisit || 0).toLocaleString()}</td>
                </tr>
            `).join('')}
        </table>
    `;
    ...
    document.getElementById('interactionInfo').innerHTML = visitor.interactions ? `
        ...
        <p><strong>Last Activity:</strong> ${new Date(visitor.interactions.lastActivity || 0).toLocaleString()}</p>
